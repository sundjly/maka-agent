...
export function buildHeavyTaskSystemPromptPolicy(
  ...
  return [
    ...
    '- Before final self_check_submit, restore verifier start state after public checks: keep solution files and task-required final deliverables, but remove, reset, or explain self-check runtime outputs that should be regenerated by the solution during verification or are not requested final deliverables. Do not delete broadly; use targeted cleanup based only on visible task/workspace evidence.',
