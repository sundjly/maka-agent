...
describe('heavy-task policy', () => {
  ...
  test('enabled policy includes public engineering behavior and official scoring separation', () => {
    ...
    assert.match(prompt, /restore verifier start state/);
    assert.match(prompt, /self-check runtime outputs/);
    assert.match(prompt, /regenerated by the solution/);
    assert.match(prompt, /task-required final deliverables/);
    assert.match(prompt, /Do not delete broadly/);
  ...
  test('verifier hygiene wording stays generic and targeted', () => {
    const policy = buildHeavyTaskSystemPromptPolicy();

    assert.match(policy, /restore verifier start state after public checks/);
    assert.match(policy, /keep solution files and task-required final deliverables/);
    assert.match(policy, /remove, reset, or explain self-check runtime outputs/);
    assert.match(policy, /Do not delete broadly; use targeted cleanup based only on visible task\/workspace evidence/);
